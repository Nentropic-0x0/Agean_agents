...
This module contains functions that return lists of system prompts, which are used to
provide context and instructions to AI agents. The prompts are designed to help the
agents understand their roles and responsibilities within the system.

The module currently includes prompts for the Threat Detection Agent, which is
responsible for analyzing network traffic and identifying potential cyber threats.
"""
class ThreatDetectionPrompts(BaseModel):
    def __init__(self, AGEAN):
        super().__init__()
        self.AGEAN = AGEAN
...
class VulnerabilityScannerPrompts(BaseModel):
    def __init__(self, AGEAN):
        super().__init__()
        self.AGEAN = AGEAN
...
class IncidenceResponsePrompts(BaseModel):
    def __init__(self, AGEAN):
        super().__init__()
        self.AGEAN = AGEAN
    ...
    def main(self):
            threat_prompts = ThreatDetectionPrompts(AGEAN).get_prompts()
            vuln_prompts = VulnerabilityScannerPrompts(AGEAN).get_prompts()
            incident_prompts = IncidenceResponsePrompts(AGEAN).get_prompts()
            return threat_prompts, vuln_prompts, incident_prompts